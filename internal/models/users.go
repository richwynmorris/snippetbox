...
func (m *UserModel) Authenticate(email, password string) (int, error) {
	// Create variables to store SQL data after query.
	var id int
	var hashedPassword []byte

	statement := `SELECT id, hashed_password FROM users WHERE email = ?`

	// Query the database and set the variable is and hashedPassword to
	// the result.
	...
	// Compare the hashedPassword and password argument provided.
